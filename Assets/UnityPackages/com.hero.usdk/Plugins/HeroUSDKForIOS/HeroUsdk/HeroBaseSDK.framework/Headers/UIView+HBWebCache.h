 ...
 */
FOUNDATION_EXPORT const int64_t HeroWebImageProgressUnitCountUnknown; /* 1LL */

typedef void(^HeroSetImageBlock)(UIImage * _Nullable image, NSData * _Nullable imageData, HBImageCacheType cacheType, NSURL * _Nullable imageURL);

/**
 Integrates HeroWebImage async downloading and caching of remote images with UIView subclass.
 */
 ...
 * See more about this in `HeroWebImageContextSetImageOperationKey`.
 ...
 * The `totalUnitCount` and `completedUnitCount` will be reset to 0 after a new image loading start (change from current queue). And they will be set to `HeroWebImageProgressUnitCountUnknown` if the progressBlock not been called but the image loading success to mark the progress finished (change from main queue).
 ...
 *
 *   The fifth parameter normally is always YES. However, if you provide HeroWebImageAvoidAutoSetImage with HeroWebImageProgressiveLoad options to enable progressive downloading and set the image yourself. This block is thus called repeatedly with a partial image. When image is fully downloaded, the
...
- (void)hb_internalSetImageWithURL:(nullable NSURL *)url
                  placeholderImage:(nullable UIImage *)placeholder
                     ...
                     setImageBlock:(nullable HeroSetImageBlock)setImageBlock
