...
/**
 A `UIImage` instance from `HBWebImageManager` when you specify `HeroWebImageRefreshCached` and image cache hit.
 ...
 @note If you don't implement `HeroWebImageRefreshCached` support, you do not need to care about this context option.
 */
FOUNDATION_EXPORT HeroWebImageContextOption _Nonnull const HeroWebImageContextLoaderCachedImage;

#pragma mark - Helper method

/**
 This is the built-in decoding process for image download from network or local file.
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with HeroWebImage's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable HBImageLoaderDecodeImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, HeroWebImageOptions options, HeroWebImageContext * _Nullable context);

/**
 This is the built-in decoding process for image progressive download from network. It's used when `HeroWebImageProgressiveLoad` option is set. (It's not required when your loader does not support progressive image loading)
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with HeroWebImage's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable HBImageLoaderDecodeProgressiveImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, BOOL finished,  id<HBWebImageOperation> _Nonnull operation, HeroWebImageOptions options, HeroWebImageContext * _Nullable context);
...
/**
 ...
 If this return YES, failed URL which does not using `HeroWebImageRetryFailed` will be blocked into black list. Else not.
